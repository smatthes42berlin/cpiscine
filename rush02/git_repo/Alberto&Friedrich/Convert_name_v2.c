...
/*   Updated: 2023/04/02 17:03:52 by smatthes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	convert_1(unsigned int number, char *result, char ***key_val_arr))
{
	...
	{
		convert_1(number / 1000000000, result);
	...
	if (number >= 1000000)
	{
		convert_1(number / 1000000, result);
	...
	if (number >= 1000)
	{
		convert_1(number / 1000, result);
		ft_strcat(result, get_num_word(1000, key_val_arr));
		// ft_strcat(result, " thousand ");
		number %= 1000;
	}
	convert_2(number, result);
}

void	convert_2(unsigned int number, char *result)
{
	if (number >= 100)
	{
		convert_1(number / 100, result);
